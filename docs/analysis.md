...
- [The Zone](https://store.steampowered.com/app/3491280/) loads a custom map as one glTF binary file (`.glb`) that [Blender](https://www.blender.org/) exported. The game runs on [Godot 4](https://godotengine.org/).4 or later. There is no SDK and there are no documented size limits. Custom map support is marked experimental.
- [Maa-amet](https://geoportaal.maaruum.ee/) publishes everything that a low fidelity city model needs: LOD2 buildings for all of [Tartu](https://en.wikipedia.org/wiki/Tartu) (22,466 objects, 11 MB), 18 points per m² lidar, 1 m elevation models, 10 cm orthophotos, 350,792 tree points, and vector data for roads, rails, water, power lines and towers. The license permits redistribution with attribution.
...
- Build order: a 1 km² test tile first, then the base map, then [Annelinn](https://et.wikipedia.org/wiki/Annelinn) and [Lohkva](https://et.wikipedia.org/wiki/Lohkva) as the first detailed map.

## 1. What The Zone accepts

Verified facts from the wiki and the [Steam](https://store.steampowered.com/app/3491280/) patch notes (see `docs/sources.md`):

- Engine: [Godot](https://godotengine.org/) 4.3 in March 2025, upgraded to 4.4, later versions not named.
...
All data is under the [Maa- ja Ruumiamet](https://geoportaal.maaruum.ee/) open data license of 2025-01-01. Commercial use, derivatives and redistribution are allowed. Attribution must name the licensor, the data set and the data date, and the license text or link must travel with redistributed data.

| Data | Detail | Format | Tartu size | Use in the map |
|------|--------|--------|------------|----------------|
| 3D buildings LOD2 | Roof shapes from lidar, footprints from [ETAK](https://geoportaal.maaamet.ee/est/ruumiandmed/eesti-topograafia-andmekogu-p79.html). No textures. | OBJ, CityGML, GDB | 22,466 objects, 11.2 MB OBJ | All buildings in the base map, background buildings in detailed maps |
...
| Oblique aerial photos | [Fotoladu](https://fotoladu.maaamet.ee/) archive, viewing service | JPEG | millions of photos | Facade texture source, see section 4 |
...
Download is by map sheet with a plain URL pattern, and Maa-amet states that there is no limit on the number of sheets. A script can fetch all Tartu sheets. The LOD2 download for Tartu was tested on 2026-09-11: 11.8 MB zip, OBJ file of 64 MB, exported with FME on 2026-09-05. The OBJ has no object groups. Each building is one `usemtl` block with a generated name, so the ETAK building ID is only in the CityGML and GDB variants. Coordinates are [L-EST97](https://epsg.io/3301) with a local offset that the `.fwt` sidecar file stores (easting 654513.288, northing 6475676.235, height 62.645), Z is up.
...
| [Mapillary](https://www.mapillary.com/) | Partial | [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/), share-alike taints the asset license | Avoid |
| [Google Street View](https://www.google.com/streetview/), Google 3D | None allowed | Terms forbid extraction and derived content | Human reference only |

Procedural facades for the first release. The GDB and CityGML variants of the LOD2 data carry the EHR code ([Ehitisregister](https://livekluster.ehr.ee/ui/ehr/v1)). The register gives the use of the building, the year and the number of floors. The pipeline picks one of a few facade sets by these attributes: panel house, brick, plaster, glass office, industrial sheet, wooden house. Floor height is about 3 m. Roof color comes from the orthophoto. Walls get a window grid and a base color per set. This is the "generic up close, Tartu from the air" look. Material names can reuse The Zone texture library so that the walls match the official maps.

Fotoladu for the second release. A documented API gives the photo metadata for one point: `https://fotoladu.maaamet.ee/api.php?B=<latitude>&L=<longitude>`, and an L-EST97 form with `x` and `y`. Maa-amet uses the same entry point in its own [QGIS plugin](https://plugins.qgis.org/plugins/estonian-oblique-aerial-photographs-release/) and in its [ArcGIS Pro add-in](https://koodivaramu.eesti.ee/maa-ja-ruumiamet/arcgis-pro-add-ins/kaldaerofoto). One point in the center of Tartu returned 199 photos from 2009 to 2024, measured on 2026-09-12. The response gives, for each photo, the file name, the tile path that names the camera body, the image size, the flight height (about 1300 m in Tartu), the date, an accuracy flag, and the four ground corners of the footprint. It does not give the camera position, the camera angles or the focal length. The newest photos of Tartu are from May 2024.

A resection recovers the missing values from the four corners alone. Measured on 2026-09-12 on the newest photos of the center of Tartu: the reprojection error is 2 to 6 pixels on a 7952 x 5304 frame, which is 0.02 to 0.07 percent of the image width. The tile path names the camera body, and the image size confirms it: `a7r3` with 7952 x 5304 pixels is the [Sony A7R III](https://en.wikipedia.org/wiki/Sony_%CE%B17R_III), which has a 35.9 x 24 mm sensor. On that sensor the focal length comes out at 84 to 85 mm, and on the A7R IV at 35 mm. The tilt is 44 to 45 degrees from nadir and the roll is near zero. The horizontal distance from the camera to the center of the footprint agrees with the tilt and the height. Two cautions. The closed form solution degenerates, because a roll near zero puts one vanishing point at infinity, so the pipeline must use a non linear fit. About one photo in three falls into a wrong local minimum, so the pipeline must reject a solution whose height or tilt is not plausible.

The recovered geometry gives 9 cm per pixel across a facade and 13 cm per pixel down it, because the facade is 46 degrees from the view ray. A 30 m wide and 15 m tall facade therefore gets about 330 x 115 pixels. This is enough for color, window rhythm and material bands, but not for detail. The full frame comes only as Deep Zoom tiles, 672 tiles for one photo. The plain preview file beside them is 1917 x 1280 pixels, which gives 38 cm per pixel across a facade. The preview is enough for one average color per building, and not for a texture.

Two conditions. First, there is no bulk download, only the viewer tiles. Write to fotoladu@maaruum.ee and ask for bulk access with orientation data for a named list of photos before any script touches the viewer. Second, the [Chancellor of Justice](https://www.oiguskantsler.ee/sites/default/files/2025-11/Orto-%20ja%20kaldaerofotode%20avalikustamine.pdf) wrote to the ministry in November 2025 about privacy in the high resolution 2025 photos. Access can tighten. Do not build the plan on Fotoladu alone.

Tartu city had two 3D models, a lidar volume model by [CafaTech](https://cafa3d.com/) and a Unity model of the extended center. The page is offline now and no reuse license was stated. Ask rlo@tartu.ee if the Unity model source is available, but do not wait for it.
...
2. [Tähtvere](https://et.wikipedia.org/wiki/T%C3%A4htvere) ja Vaksali põhjaots. The laululava, the 330 kV substation with lines in four directions, [Tartu Näitused](https://tartunaitused.ee/). Best wire content. The laululava shell is the first hand made hero asset.
3. [Kesklinn](https://et.wikipedia.org/wiki/Kesklinn_%28Tartu%29) ja [Emajõgi](https://et.wikipedia.org/wiki/Emaj%C3%B5gi). The most recognizable and the hardest. Spires and the Kaarsild need hand work.
4. [Raadi](https://et.wikipedia.org/wiki/Raadi_lennuv%C3%A4li). ERM and the runway, cheap to build, good for speed.
5. Vaksali, Maarjamõisa ja Tammelinn. Station, [Tartu Mill](https://tartumill.ee/), hospital, telemast.
6. [Ülejõe](https://et.wikipedia.org/wiki/%C3%9Clej%C3%B5e) ja [Supilinn](https://et.wikipedia.org/wiki/Supilinn). [Lodjakoda](https://lodi.ee/) and the [Haine Paelavabrik](https://www.haine.ee/) block. Can merge with Kesklinn later as one 6 km² map if the game handles it.
7. Ropka, [Ihaste](https://et.wikipedia.org/wiki/Ihaste) ja [Aardlapalu](https://et.wikipedia.org/wiki/Aardlapalu). Quarry and river meanders.
8. [Lõunakeskus](https://lounakeskus.com/).
...
- Docker image with [GDAL](https://gdal.org/), [PDAL](https://pdal.io/), Python 3.12, [geopandas](https://geopandas.org/), shapely, rasterio, laspy, [trimesh](https://trimesh.org/), [pygltflib](https://pypi.org/project/pygltflib/).
...
- Coordinate system: L-EST97 (EPSG:3301) is already metric. The map origin is the center of the bounding box, Y is up, and heights use the [EH2000](https://geoportaal.maaamet.ee/est/ruumiandmed/geodeetilised-andmed/korgussusteem-eh2000-p1049.html) datum as they come.
